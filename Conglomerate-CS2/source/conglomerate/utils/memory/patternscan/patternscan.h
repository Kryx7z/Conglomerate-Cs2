...
namespace M {
	...
	{
		// Every caller feeds this straight from FindPattern, so a failed scan
		// used to walk through the null pointer while reading the relative
		// displacement below.
		if (relative_address == nullptr)
			return nullptr;
